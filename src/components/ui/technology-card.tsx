...
export function TechnologyCard({ title, description, icon, link, gradient }: TechnologyCardProps) {
  return (
    <Link href={link} className="block group h-full">
      <div className="relative bg-[#252525] rounded-xl p-8 transition-all duration-500 border border-gray-600 hover:border-rose-500 hover:shadow-xl hover:shadow-rose-500/20 hover:-translate-y-2 overflow-hidden h-full flex flex-col">
        ...
        <div className="relative z-10 text-center flex flex-col flex-1">
          ...
          <p className="text-gray-300 group-hover:text-white transition-colors duration-300 flex-1 min-h-[60px]">
            {description}
          </p>
...
export function CardGrid({ children, columns = 3, className = '' }: CardGridProps) {
  ...
  return (
    <div className={`grid grid-cols-1 ${colsClass[columns]} gap-8 ${className}`}>
      {children}
    </div>
  );
}
