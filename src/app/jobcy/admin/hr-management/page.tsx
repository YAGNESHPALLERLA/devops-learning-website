...
export default function HRManagement() {
  ...
  useEffect(() => {
    const fetchHRs = async () => {
      try {
        const res = await fetch("/api/jobcy-backend/admin/hrs", {
          headers: {
    ...
    const fetchCompanies = async () => {
      try {
        const res = await fetch("/api/jobcy-backend/admin/companies", {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
...
const handleInputChange = (
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (currentView === "create") {
        const res = await fetch("/api/jobcy-backend/admin/create-hr", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const res = await fetch(
    `/api/jobcy-backend/admin/hrs/${selectedHR._id}`,
    {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to delete this HR user?")) {
      try {
        const res = await fetch(`/api/jobcy-backend/admin/hrs/${id}`, {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
