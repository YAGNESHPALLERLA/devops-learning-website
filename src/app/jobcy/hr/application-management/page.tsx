  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        ...
        const response = await fetch(`${"/api/jobcy-backend"}/hr/applications`, {
          headers: {
...
const getStatusIcon = (status: "pending" | "shortlisted" | "rejected") => {
  ...
  const updateApplicationStatus = async (
    ...
    try {
      ...
      const response = await fetch(`${"/api/jobcy-backend"}/hr/applications/${applicationId}/status`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
          ...
          {application.hasResume && (
            <button
              onClick={async () => {
                try {
                  const token = localStorage.getItem("token");
                  const response = await fetch(`${"/api/jobcy-backend"}/hr/resume/${application.userId}`, {
                    headers: {
                      Authorization: `Bearer ${token}`,
                    },
                  });
              ...
              <div className="mb-6">
                ...
                <button
                  onClick={async () => {
                    try {
                      const token = localStorage.getItem("token");
                      const response = await fetch(`${"/api/jobcy-backend"}/hr/resume/${applicant.userId}`, {
                        headers: {
                          Authorization: `Bearer ${token}`,
                        },
                      });
