...
# Fix escaped quotes
content = content.replace("getImages(\\'image18\\'", "getImages('image18'")
content = content.replace("getImages(\\'image21\\'", "getImages('image21'")
content = content.replace("getImages(\\'image22\\'", "getImages('image22'")
content = content.replace("getImages(\\'image47\\'", "getImages('image47'")
content = content.replace("getImages(\\'image48\\'", "getImages('image48'")
content = content.replace("\\', \\'", "', '")
content = content.replace("\\')", "')")

# Fix 1: Add image16, image17 after "Check for the Storage Account" in Blob Storage section
# Find the first occurrence (Blob Storage section)
blob_check_match = re.search(
    r'(<p className="font-semibold mt-4">Check for the Storage Account</p>.*?<p className="text-green-400">If it appears there.*?</p>\s*</div>\s*<ImageGallery images=\{getImages\(\'image44\'\)\} />)',
    content,
    re.DOTALL
)
if blob_check_match:
    blob_text = blob_check_match.group(1)
    new_blob_text = blob_text.replace(
        "<ImageGallery images={getImages('image44')} />",
        "<ImageGallery images={getImages('image16', 'image17')} />"
    )
    content = content.replace(blob_text, new_blob_text)

# Fix 2: Add image18, image21, image22, image47, image48 after "Click Create" in Explore Blob Service
content = re.sub(
...
# Fix 3: Fix "Change the access tier" section
content = re.sub(
...
# Fix 4: Replace image52 with Archive images
content = content.replace(
...
# Fix 5: Remove duplicate image53
content = content.replace(
    "</table>\n            </div>\n\n            <ImageGallery images={getImages('image53')} />",
    "</table>\n            </div>"
)

# Fix 6: Fix ADLS section
...
print("Fixed all images and escaped quotes!")
